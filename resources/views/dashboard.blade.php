        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 p-4 rounded-lg bg-gray-50 border border-gray-200" role="alert">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('notifications.index') }}" class="text-sm font-medium text-orange-500 hover:text-orange-600">View all notifications →</a>
                </div>
            </div>
            ...
            <div class="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700" role="alert">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium">You have {{ $unreadNotifications }} unread notification{{ $unreadNotifications > 1 ? 's' : '' }}</span>
                    <a href="{{ route('notifications.index') }}" class="ml-auto text-sm font-medium text-red-600 hover:text-red-800">View all →</a>
                </div>
            </div>
            @endif
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-green-600 mt-1">{{ $stats['available_vehicles'] }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-blue-600 mt-1">{{ $stats['vehicles_in_use'] }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-teal-600 mt-1">{{ $stats['avg_fuel_consumption'] }} <span class="text-lg">km/L</span></p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-purple-600 mt-1">Rp {{ number_format($stats['total_fuel_cost'] ?? 0, 0, ',', '.') }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-orange-600 mt-1">{{ $stats['pending_requests'] }}</p>
                        </div>
                        <div class="p-3 bg-orange-100 rounded-lg">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-blue-600 mt-1">{{ $stats['today_trips'] }}</p>
                        </div>
                        <div class="p-3 bg-blue-100 rounded-lg">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $stats['completed_trips'] }}</p>
                        </div>
            ...
            @if(Auth::user()->role === 'admin' && count($vehicles) > 0)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-8 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900">Quick Vehicle Status</h3>
                    <a href="{{ route('vehicles.index') }}" class="text-sm text-orange-500 hover:text-orange-600 font-medium">View All →</a>
                </div>
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 font-medium text-gray-900">{{ $vehicle->name }}</td>
                                <td class="px-6 py-4"><span class="font-mono text-sm bg-gray-100 px-2 py-1 rounded">{{ $vehicle->plate_number }}</span></td>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <a href="{{ route('requests.index') }}" class="text-sm text-orange-500 hover:text-orange-600 font-medium">View All →</a>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-50 text-gray-600 font-medium">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium
                                        ...
                                        @elseif($req->status === 'manager_approved' || $req->status === 'admin_approved') bg-orange-100 text-orange-800
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('requests.show', $req->id) }}" class="text-orange-500 hover:text-orange-600 font-medium">View</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
            ...
            @if(Auth::user()->role === 'manager' && isset($deptTrips))
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mt-6">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($deptTrips as $trip)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 font-medium">#{{ $trip->id }}</td>
                                <td class="px-6 py-4">{{ $trip->borrower->name ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $trip->borrower->department ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $trip->vehicle->name ?? '-' }} ({{ $trip->vehicle->plate_number ?? '-' }})</td>
                                <td class="px-6 py-4 max-w-xs truncate">{{ $trip->destination }}</td>
                                <td class="px-6 py-4">{{ $trip->start_datetime->format('d M Y H:i') }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium
                                        @if($trip->status === 'pending') bg-yellow-100 text-yellow-800
                                        @elseif($trip->status === 'driver_cancelled') bg-red-100 text-red-800
                                        @elseif($trip->status === 'completed') bg-green-100 text-green-800
                                        @elseif($trip->status === 'in_progress' || $trip->status === 'manager_approved') bg-blue-100 text-blue-800
                                        @elseif($trip->status === 'admin_approved') bg-orange-100 text-orange-800
                                        @else bg-gray-100 text-gray-800
                                        @endif">
                                        {{ str_replace(['_', ' '], ' ', ucwords(str_replace('_', ' ', $trip->status))) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('requests.show', $trip->id) }}" class="text-orange-500 hover:text-orange-600 font-medium">View</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
